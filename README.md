## Olá! Eu sou o Gabs 👋

### 💻 Desenvolvedor Fullstack | Java | Node.js | Angular

- 🔧 Trabalho com microsserviços, APIs REST, integrações e automações
- 🌐 Experiência com bancos relacionais e NoSQL (PostgreSQL, SQL Server, MongoDB)
- ☁️ Familiarizado com Docker, AWS, CI/CD e arquitetura em nuvem
- 📚 Estudando constantemente e buscando novos desafios

---

### 🛠️ Tecnologias

Java · Spring Boot · Node.js · Angular · React · TypeScript · PostgreSQL · MongoDB · Docker · Git · AWS · Kafka

---
[![My Skills](https://skillicons.dev/icons?i=python,docker,fastapi,pytorch,tensorflow,ubuntu,git,aws,postgresql,mysql)](https://skillicons.dev)

### 📫 Contato

[LinkedIn](https://www.linkedin.com/in/gabriel-fullstack-dev)  
[E-mail](gabrielolivernas@gmail.com) 
